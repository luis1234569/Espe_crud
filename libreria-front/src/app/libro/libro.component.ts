...
export class LibroComponent implements OnInit {
  ...
  save(): void {
    this.bookService.save(this.initialValues).subscribe(() => {
      this.initialValues = {
        libroId: 0,
        isbn: '',
        tittle: '',
  ...
  onInputAutor(term: string) {
    ...
    if (term == '') {
      ...
      this.bookService.findByAutor(term).subscribe((response) => {
        this.allBooks = response;
      });
    }
  }
  ...
  onInputTittle(term: string) {
    ...
    if (term == '') {
      ...
      this.bookService.findByTittle(term).subscribe((response) => {
        this.allBooks = response;
      });
    }
  }
  ...
  onInputEditorial(term: string) {
    ...
    if (term == '') {
      ...
      this.bookService.findByEditorial(term).subscribe((response) => {
        this.allBooks = response;
      });
    }
  }
  ...
  onInputCategoria(term: string) {
    ...
    if (term == '') {
      ...
      this.bookService.findByCategory(term).subscribe((response) => {
        this.allBooks = response;
      });
    }
  }
